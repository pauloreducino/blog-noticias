...
export function AdBanner({ size = 'rectangle', imageUrl, linkUrl, altText, label = 'PUBLICIDADE', className = '' }: AdBannerProps) {
  ...
  const inner = (
    <div
      className={`relative border border-dashed border-white/8 rounded-lg overflow-hidden flex items-center justify-center bg-surface ${className}`}
      style={{ width: w, height: h }}
      role="complementary"
      aria-label="Espaço publicitário"
    >
      <span className="absolute top-1.5 left-2 font-mono text-[9px] tracking-widest text-text-muted uppercase opacity-60">
        {label}
      </span>
      {imageUrl ? (
